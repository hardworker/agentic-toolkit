...
- **Stub-runtime smoke test (`eval/crucible-smoke.mjs`)** — executes the real script's control flow with canned agent responses (32 checks: chaining, gates, dry runs, blocked tasks, stagnation, refute kills, budget floors, error surfacing) at zero token cost.
