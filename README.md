...
- **A browser window never appears unexplained**, whichever layer opened it. It lands on a local page naming the app, the session and the triggering request, and the same line goes to the log. The client names itself; one that cannot — pointed at a fixed loopback port — is named by its route.
